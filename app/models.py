...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    """ define TABLE-message AND TABLE-user Relationship"""
    ...
    """ define TABLE-task AND TABLE-user Relationship"""
    ...
    def get_reset_password_token(self, expires_in = 600):
        ...
        # 第一个是payload，这是认证依据的主要信息
            # “exp”: 过期时间
            # “nbf”: 表示当前时间在nbf里的时间之前，则Token不被接受
            # “iss”: token签发者
            # “aud”: 接收者
            # “iat”: 发行时间
        # 第二个是密钥，这里是读取配置文件中的SECRET_KEY配置变量
        # 第三个是生成Token的算法
        return jwt.encode(
            {
    ...
    def verify_reset_password_token(token):
        # 这里的Token验证主要包括过期时间验证和声明验证。使用jwt的decode方法解析Token，得到payload。如：
    ...
    def check_token(token):
        """ a static method that takes a token as input \
         and returns the user this token belongs to as a response"""
